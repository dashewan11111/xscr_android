...
import com.adult.android.model.NewOrderDetailResponse;
...
public class OrderDetailsActivity2 extends BaseActivity implements
		OnClickListener {
	...
	private TextView txtReceiver, txtReceiverMobile, txtAddress,
			txtOrderAmount, txtTransFee, txtCouponFee;
	...
	private void initViews() {
		...
		findViewById(R.id.order_commit_layout_address_new).setVisibility(
				View.GONE);
	...
	private void addAllProduct() {
		...
		((TextView) findViewById(R.id.order_details_prodcut_count)).setText("共"
				+ orderDetailsDto.getSkuList().size() + "件");
		for (Sku2 sku : orderDetailsDto.getSkuList()) {
			...
			mBitmapUtils.display(imageProduct,
					ServiceUrlConstants.getImageHost() + sku.getImgUrl());
	...
	protected void getDateList() {
		LoadingDialog.show();
		OrderModel2.getInstance().getOrderDetail(orderId,
				new OnGetOrderDetailCompletedListener() {
					...
					public void onFailed(ResponseException e) {
						LoadingDialog.dismiss();
						ToastUtil.showToastShort(OrderDetailsActivity2.this,
								e.getResultMsg());
	...
	protected void initOrder() {
		txtOrderAmount.setText(getResources().getString(R.string.rmb)
				+ orderDetailsDto.getOrderAmount());
		txtTransFee.setText(getResources().getString(R.string.rmb)
				+ orderDetailsDto.getTransferAmount());
		txtCouponFee.setText(getResources().getString(R.string.rmb)
				+ orderDetailsDto.getCouponAmount());
	}
	...
	protected void addAddressInfo() {
		...
		txtAddress.setText(addressInfo.getProvinceName() + " "
				+ addressInfo.getCityName() + " " + addressInfo.getAreaName()
		...
		case R.id.order_details_btn_delete:
			...
			dialog.setTwoButton(R.string.cancel, R.color.black,
					...
					new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface arg0, int arg1) {
							LoadingDialog.show();
							OrderModel2.getInstance().deleteOrder(orderId,
