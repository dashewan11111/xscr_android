...
public class BGABanner extends RelativeLayout {
	...
	private void initAttrs(Context context, AttributeSet attrs) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (attr == R.styleable.BGABanner_pointFocusedImg) {
				 ...
				 */
				mPointSpacing = typedArray.getDimensionPixelSize(attr,
						mPointSpacing);

			} else if (attr == R.styleable.BGABanner_pointEdgeSpacing) {
				mPointEdgeSpacing = typedArray.getDimensionPixelSize(attr,
						mPointEdgeSpacing);
				...
				try {
					mPointContainerWidth = typedArray.getDimensionPixelSize(
							attr, mPointContainerWidth);
				} catch (UnsupportedOperationException e) {
					// 如果是指定的wrap_content或者match_parent会执行下面这一行
					mPointContainerWidth = typedArray.getInt(attr,
							mPointContainerWidth);
				}

			} else if (attr == R.styleable.BGABanner_pointContainerHeight) {
				try {
					mPointContainerHeight = typedArray.getDimensionPixelSize(
							attr, mPointContainerHeight);
				} catch (UnsupportedOperationException e) {
					mPointContainerHeight = typedArray.getInt(attr,
							mPointContainerHeight);
				}

			} else if (attr == R.styleable.BGABanner_pointVisibility) {
				mPointVisibility = typedArray
						.getBoolean(attr, mPointVisibility);
				...
				mAutoPlayInterval = typedArray.getInteger(attr,
						mAutoPlayInterval);
			}
		}
		typedArray.recycle();
		...
		if (mPointVisibility) {
			if (mPointFocusedDrawable == null) {
				throw new RuntimeException(
						"pointFocusedImg is not allowed to be NULL");
			} else if (mPointUnfocusedDrawable == null) {
				throw new RuntimeException(
						"pointUnfocusedImg is not allowed to be NULL");
			}
			mPointContainer = new LinearLayout(context);
			mPointContainer.setOrientation(LinearLayout.HORIZONTAL);
			mPointContainer.setPadding(mPointEdgeSpacing, 0, mPointEdgeSpacing,
					0);
			...
			RelativeLayout.LayoutParams pointContainerLp = new RelativeLayout.LayoutParams(
					mPointContainerWidth, mPointContainerHeight);
			// 处理圆点在顶部还是底部
			...
			if (horizontalGravity == Gravity.LEFT) {
				mPointContainer.setGravity(Gravity.LEFT
	...
	private void processAutoPlay() {
		if (mAutoPlayAble) {
			...
			mViewPager.setCurrentItem(Integer.MAX_VALUE / 2
					- (Integer.MAX_VALUE / 2) % mViews.size());
		} else {
			switchToPoint(0);
		}
	}
	...
	private final class MyAdapter extends PagerAdapter {
		...
		public Object instantiateItem(ViewGroup container, int position) {
			View view = null;
			Log.e("instantiateItem", "position : " + position + " , size : "
					+ mViews.size() + " , " + position % mViews.size());
			if (mAutoPlayAble) {
