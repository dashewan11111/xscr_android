...
public class RoundProgressBar extends View {
	...
	public RoundProgressBar(Context context, AttributeSet attrs, int defStyle) {
		...
		textSize1 = mTypedArray.getDimension(
				R.styleable.RoundProgressBar_textSize1,
				Misc.sp2px(getContext(), 17));
		textSize2 = mTypedArray.getDimension(
				R.styleable.RoundProgressBar_textSize2,
				Misc.sp2px(getContext(), 11));
	...
	protected void onDraw(Canvas canvas) {
		...
		paint.setTypeface(Typeface.DEFAULT_BOLD); // 设置字体
		float sampleTextWidth = paint.measureText("好评度"); // 测量字体宽度，我们需要根据字体的宽度设置在圆环中间

		if (textIsDisplayable && style == STROKE) {
			canvas.drawText(sampleText, centre - sampleTextWidth / 2, centre
					+ textSize2, paint); // 画出进度百分比
		}
